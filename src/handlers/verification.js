...
export function buildWhitelistModal() {
  const modal = new ModalBuilder()
    .setCustomId("modal_whitelist_rp")
    .setTitle("Whitelist RP — Black");
  ...
  const idade = new TextInputBuilder()
    .setCustomId("wl_idade")
    .setLabel("Idade")
    .setStyle(TextInputStyle.Short)
    .setRequired(true);

  const steam = new TextInputBuilder()
    .setCustomId("wl_steamid")
    .setLabel("Steam ID (64)")
    .setStyle(TextInputStyle.Short)
    .setRequired(true);

  const exp = new TextInputBuilder()
    .setCustomId("wl_exp")
    .setLabel("Experiência com RP (Sim/Não)")
    .setStyle(TextInputStyle.Short)
    .setRequired(true);

  const historia = new TextInputBuilder()
    .setCustomId("wl_hist")
    .setLabel("História do personagem (até 250 chars)")
